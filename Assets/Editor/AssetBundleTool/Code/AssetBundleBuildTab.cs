...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private string m_streamingPath = "Assets/StreamingAssets/Bundles";
        ...
        ToggleData m_CopyAllToStreaming;
        ToggleData m_CopyIncludeToStreaming;
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_ForceRebuild = new ToggleData(
                ...
                m_UserData.m_OnToggles);
            m_CopyAllToStreaming = new ToggleData(
                false,
                "Copy All to StreamingAssets",
                "After build completes, will copy all build content to " + m_streamingPath + " for use in stand-alone player.",
                m_UserData.m_OnToggles);

            m_CopyIncludeToStreaming = new ToggleData(
                false,
                "Copy Included to StreamingAssets",
                "After build completes, will copy included build content to " + m_streamingPath + " for use in stand-alone player.",
                m_UserData.m_OnToggles);
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope (!AssetBundleModel.Model.DataSource.CanSpecifyBuildTarget)) {
                ...
                if (tgt != m_UserData.m_BuildTarget)
                {
                    ...
                    if(m_UserData.m_UseDefaultPath)
                    {
                        ...
                        //m_UserData.m_OutputPath += m_UserData.m_BuildTarget.ToString() + "/Bundles";
            ...
            using (new EditorGUI.DisabledScope (!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)) {
                ...
                newState = GUILayout.Toggle(
                    m_CopyAllToStreaming.state,
                    m_CopyAllToStreaming.content);
                if (newState != m_CopyAllToStreaming.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_CopyAllToStreaming.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_CopyAllToStreaming.content.text);
                    m_CopyAllToStreaming.state = newState;
                }

                newState = GUILayout.Toggle(
                    m_CopyIncludeToStreaming.state,
                    m_CopyIncludeToStreaming.content);
                if (newState != m_CopyIncludeToStreaming.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_CopyIncludeToStreaming.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_CopyIncludeToStreaming.content.text);
                    m_CopyIncludeToStreaming.state = newState;
                }
            }
            ...
            //if (GUILayout.Button("Split Build"))
            //{
            //    ManifestDataFile.Instance.UpdateData();   
            //}
        ...
        private void ExecuteBuild()
        {
            ...
            if (m_CopyAllToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath , true, false);
            if (m_CopyIncludeToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath, false, true);

        }

        private static void DirectoryCopy(string sourceDirName, string destDirName ,  bool all , bool include)
        {
            ...
            foreach (string filePath in Directory.GetFiles(sourceDirName, "*.*", SearchOption.AllDirectories))
            {
                ...
                else if (include)
                {
                    ...
                    if (bundleName.Contains(".manifest"))
                    {
                        bundleName = bundleName.Replace(".manifest", "");
                    }

                    ResourcesManifestData.Bundle bundle = ManifestDataFile.Instance.GetBundle(bundleName);
