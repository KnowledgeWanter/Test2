...
namespace Zone.AB
{
    public class ComparisonUtils
    {
                                    ...
                                    , ResourcesManifest resourcesmanifest)
        {
            ...
            var itr = resourcesmanifest.Data.Bundles.GetEnumerator();
            while (itr.MoveNext())
            {
                if (itr.Current.Included)
                {
                    string name = itr.Current.Name;
                                                ...
                                                , emCompareMode compareMode)
        {
            ...
            //标记旧的本地资源
            if (old_resourcesmanifest != null && old_resourcesmanifest.Data.Bundles != null)
            {
                var resource_manifest_itr = old_resourcesmanifest.Data.Bundles.GetEnumerator();
                while (resource_manifest_itr.MoveNext())
                {
                    if (resource_manifest_itr.Current.Included)
                    {
                        string name = resource_manifest_itr.Current.Name;
            ...
            //标记新的本地资源
            if (new_resourcesmanifest != null && new_resourcesmanifest.Data.Bundles != null)
            {
                var resource_manifest_itr = new_resourcesmanifest.Data.Bundles.GetEnumerator();
                while (resource_manifest_itr.MoveNext())
                {
                    if (resource_manifest_itr.Current.Included)
                    {
